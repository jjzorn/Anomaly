...
Client::Client(const std::string& hostname, uint16_t port, Renderer& renderer) {
	if (enet_initialize() < 0) {
		renderer.error("Network initialization failed");
	}
	host = enet_host_create(nullptr, 1, NET_CHANNELS, 0, 0);
	if (host == nullptr) {
		renderer.error("Could not create network socket");
	}
	ENetAddress address = { 0 };
	if (enet_address_set_host(&address, hostname.c_str()) < 0) {
		renderer.error("Could not resolve hostname '" + hostname + "'");
	}
	address.port = port;
	peer = enet_host_connect(host, &address, NET_CHANNELS, 0);
	ENetEvent event;
	if (enet_host_service(host, &event, 5000) == 0 || event.type != ENET_EVENT_TYPE_CONNECT) {
		renderer.error("Could not connect to '" + hostname + "[" + std::to_string(port) + "]'");
	}
}
